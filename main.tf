...
###################################################
# ✅ Week 1 - 1.3: Create security group for Bastion Host
...
###################################################
# ✅ Week 3 - 3.1 North-South: Aparte Loadbalancer SG
# Deze staat open voor het internet (North-South), de webservers zelf niet meer.
resource "aws_security_group" "alb_sg" {
  name        = "alb_sg"
  description = "Allow HTTP from internet to Application Load Balancer"
  vpc_id      = aws_vpc.saxit_vpc.id
...
resource "aws_security_group" "presentationtier_sg" {
 ...
 # ✅ Week 1 - 1.3: SSH alleen via Bastion toegestaan
 ingress {
   description     = "SSH ingress from Bastion"
   from_port       = 22
   to_port         = 22
   protocol        = "tcp"
   security_groups = [aws_security_group.bastion_sg.id] 
 }
 # ✅ Week 3 - 3.1 North-South: Microsegmentatie toegepast. 
 # Webservers accepteren alleen nog maar webverkeer vanaf de Load Balancer.
 ingress {
...
resource "aws_security_group" "applicationtier_sg" {
 ...
 description = "Allow SSH from Bastion and 8080 from Webservers"
 vpc_id      = aws_vpc.saxit_vpc.id
 ...
 # Appservers accepteren alleen verkeer vanuit de Presentation Tier (Webservers).
 ingress {
   description     = "HTTP ingress from Presentation Tier"
   from_port       = 8080
   to_port         = 8080
   protocol        = "tcp"
...
resource "aws_subnet" "saxit_subnet_presentation_1" {
  ...
  tags = { Name  = "saxit_subnet_presentation_1" }
}
...
resource "aws_subnet" "saxit_subnet_presentation_2" {
  ...
  tags = { Name  = "saxit_subnet_presentation_2" }
}
...
resource "aws_subnet" "saxit_subnet_appl_2" {
  ...
  availability_zone = "us-east-1b"
  tags = { Name  = "saxit_subnet_appl_2" }
}

###################################################
# Connect routing table to subnets
...
##################################################
# ✅ Week 1 - 1.3: Create an EC2 instance as Bastion Host
...
resource "aws_instance" "app01" {
  ...
  sudo docker run --restart always -e SPRING_DATASOURCE_URL=jdbc:mysql://terraform-20251009091928043800000001.cnjxggafdz1n.us-east-1.rds.amazonaws.com/cloudsecdb -e SPRING_DATASOURCE_USERNAME=admin -e SPRING_DATASOURCE_PASSWORD=password123 -p 8080:8080 -d backend
  EOF
  key_name = local.sshkey
  tags = { Name = "app01" }
}
...
resource "aws_lb" "presentation-lb" {
  ...
  # ✅ Week 3 - 3.1 North-South: Koppel de nieuwe internet-facing SG
  security_groups    = [aws_security_group.alb_sg.id]
...
###################################################
# Create loadbalancer application tier
